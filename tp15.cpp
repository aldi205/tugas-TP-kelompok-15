...
void cartMenu();
void initCart();
bool addToCart(const string& code, int qty);
void displayCart();
bool removeCartItem(int idx);
bool updateCartQty(int idx, int qty);
...
void printReceiptFile(int id);
// membership/search prototypes
bool memberExists(const string& uname);
void registerMember();
void listMembers();
void listMemberHistory(const string& uname);
...
void registerMember() {
    string uname = inputLine("Username member: "); if (uname.empty()) { cout<<"Username kosong.\n"; return; }
    if (memberExists(uname)) { cout<<"Member sudah terdaftar.\n"; return; }
    string name = inputLine("Nama lengkap: ");
    string phone = inputLine("No. telp: ");
    ofstream fout(MEMBERS_FILE.c_str(), ios::app);
    if (!fout) { cout<<"Gagal menulis file member.\n"; return; }
    fout << uname << '|' << name << '|' << phone << '\n';
    cout << "Member terdaftar.\n";
}

void listMembers() {
    ifstream fin(MEMBERS_FILE.c_str()); if (!fin) { cout<<"Belum ada member.\n"; return; }
    string line; int idx=1; cout<<"\nDaftar member:\n";
    while (getline(fin,line)) { if (line.empty()) continue; size_t p1=line.find('|'); size_t p2=line.find('|',p1+1); if (p1==string::npos||p2==string::npos) continue; string u=line.substr(0,p1); string name=line.substr(p1+1,p2-p1-1); string phone=line.substr(p2+1); cout<<idx++<<". "<<u<<" | "<<name<<" | "<<phone<<"\n"; }
}

void listMemberHistory(const string& uname) {
    ifstream fin(MEMBER_HISTORY_FILE.c_str()); if (!fin) { cout<<"Belum ada riwayat.\n"; return; }
    string line; cout<<"\nRiwayat untuk "<<uname<<":\n";
    while (getline(fin,line)) { if (line.empty()) continue; size_t p1=line.find('|'); if (p1==string::npos) continue; string u=line.substr(0,p1); if (u!=uname) continue; size_t p2=line.find('|',p1+1); size_t p3=line.find('|',p2+1); size_t p4=line.find('|',p3+1); string tid=line.substr(p1+1,p2-p1-1); string date=line.substr(p2+1,p3-p2-1); string total=line.substr(p3+1); cout<<"Transaksi "<<tid<<" | "<<date<<" | Rp"<<total<<"\n"; }
}

void searchProducts(const string& q) {
    ifstream fin(PRODUCTS_FILE.c_str()); if (!fin) { cout<<"File produk tidak ditemukan.\n"; return; }
    ...
    while (getline(fin,line)) {
        ...
        if (p1==string::npos||p2==string::npos||p3==string::npos||p4==string::npos||p5==string::npos) continue;
        string kode=line.substr(0,p1); string nama=line.substr(p1+1,p2-p1-1); string kategori=line.substr(p2+1,p3-p2-1); string harga=line.substr(p3+1,p4-p3-1); string stok=line.substr(p4+1,p5-p4-1); string expired=line.substr(p5+1);
        if (kode.find(q)!=string::npos || nama.find(q)!=string::npos || kategori.find(q)!=string::npos) {
            cout<<"- "<<kode<<" | "<<nama<<" | "<<kategori<<" | Rp"<<harga<<" | stok:"<<stok<<" | exp:"<<expired<<"\n"; found++; }
    }
    if (!found) cout<<"Tidak ditemukan produk yang cocok.\n";
}
int main() {
...
void listProducts() {
    ifstream fin(PRODUCTS_FILE.c_str());
    if (!fin) { cout << "Belum ada produk atau file tidak tersedia.\n"; return; }
    string line;
    int idx = 1;
    cout << "\nDaftar produk:\n";
    while (getline(fin, line)) {
        if (line.empty()) continue;
        // format: kode|nama|kategori|harga|stok|expired
        size_t p1 = line.find('|');
        size_t p2 = line.find('|', p1 + 1);
        size_t p3 = line.find('|', p2 + 1);
        size_t p4 = line.find('|', p3 + 1);
        ...
        cout << idx++ << ". " << kode << " | " << nama << " | " << kategori << " | Rp" << harga << " | stok:" << stok << " | exp:" << expired << "\n";
    }
    if (idx == 1) cout << "Belum ada produk.\n";
}
...
void ensureDefaultData() {
    ...
    if (needProducts) {
        ...
        if (pf) {
            ...
            pf << "P0001|Lipstick Red|Bibir|75000|25|2026-12-31\n";
            pf << "P0002|Lipstick Pink|Bibir|70000|30|2026-11-30\n";
            pf << "P0003|Mascara Black|Mata|90000|15|2027-01-15\n";
            pf << "P0004|Eyeliner Liquid|Mata|65000|18|2026-10-10\n";
            pf << "P0005|Face Cleanser|Wajah|45000|40|2027-06-30\n";
            pf << "P0006|Moisturizer|Wajah|120000|22|2027-08-01\n";
            pf << "P0007|Body Lotion|Bodycare|60000|35|2027-03-20\n";
            pf << "P0008|Hand Cream|Bodycare|40000|50|2027-05-10\n";
            pf << "P0009|Serum Vitamin C|Skincare|150000|12|2027-02-28\n";
            pf << "P0010|Sunscreen SPF50|Skincare|95000|28|2027-07-31\n";
            pf << "P0011|Blush Peach|Wajah|55000|20|2026-12-01\n";
            pf << "P0012|BB Cream|Wajah|85000|14|2027-04-15\n";
            pf << "P0013|Face Mask Clay|Skincare|30000|60|2028-01-01\n";
            pf << "P0014|Lip Balm|Bibir|20000|80|2028-06-30\n";
            pf << "P0015|Eye Shadow Palette|Mata|180000|10|2027-09-30\n";
            pf << "P0016|Facial Toner|Skincare|50000|45|2027-03-31\n";
            pf << "P0017|Makeup Remover|Wajah|38000|33|2027-05-05\n";
            pf << "P0018|Body Scrub|Bodycare|48000|26|2027-11-11\n";
            pf << "P0019|Concealer|Wajah|67000|19|2026-12-25\n";
            pf << "P0020|Facial Oil|Skincare|140000|9|2027-08-20\n";
            ...
            for (int i = 21; i <= 40; ++i) {
                ...
                int stock = 10 + (i%7)*5;
                string exp = (i%2==0)?"2027-12-31":"2028-06-30";
...
bool addToCart(const string& code, int qty) {
    ...
    while (getline(fin, line)) {
        ...
        size_t p4 = line.find('|', p3 + 1);
        size_t p5 = line.find('|', p4 + 1);
        if (p2==string::npos||p3==string::npos||p4==string::npos||p5==string::npos) continue;
        string nama = line.substr(p1+1, p2-p1-1);
        string kategori = line.substr(p2+1, p3-p2-1);
        string harga = line.substr(p3+1, p4-p3-1);
        string stok = line.substr(p4+1, p5-p4-1);
        string expired = line.substr(p5+1);
        double price = 0.0;
        try { price = stod(harga); } catch(...) { price = 0.0; }
        // check if product already in cart
        for (int i = 0; i < cartCount; ++i) {

            if (cartCode[i] == code) {
                cartQty[i] += qty;
                cartSubtotal[i] = cartQty[i] * price;
                return true;
            }
        }
...
void displayCart() {
    ...
    for (int i = 0; i < cartCount; ++i) {
        double price = 0.0;
        try { price = stod(cartPriceStr[i]); } catch(...) { price = 0.0; }
        cout << (i+1) << ". " << cartCode[i] << " | " << cartName[i] << " | Rp" << cartPriceStr[i]
             << " | qty: " << cartQty[i] << " | subtotal: " << cartSubtotal[i] << "\n";
...
bool removeCartItem(int idx) {
    ...
    for (int j = i; j < cartCount - 1; ++j) {
        ...
        cartPriceStr[j] = cartPriceStr[j+1];
...
void saveTransactionToFile(const string& date, double total, const string& items, const string& member, double paid, double change, double discount, const string& promoCode) {
    int id = 1;
    ifstream fin(TRANSACTIONS_FILE.c_str()); string line; while (fin && getline(fin,line)) { if (!line.empty()) id++; } fin.close();
    ofstream fout(TRANSACTIONS_FILE.c_str(), ios::app); if (!fout) return;
    // format: id|date|total|discount|final|paid|change|promo|member|items
    double finalAmount = total - discount;
    fout << id << '|' << date << '|' << (int)total << '|' << (int)discount << '|' << (int)finalAmount << '|' << (int)paid << '|' << (int)change << '|' << promoCode << '|' << member << '|' << items << '\n';
    if (!member.empty()) {
        ofstream fh(MEMBER_HISTORY_FILE.c_str(), ios::app);
        if (fh) fh << member << '|' << id << '|' << date << '|' << (int)finalAmount << '\n';
    }
}
...
bool checkoutCart(const string& curDate) {
    ...
    for (int ci = 0; ci < cartCount; ++ci) {
        ...
        if (!curDate.empty() && expired <= curDate) { cout << "Produk " << code << " sudah expired/tidak layak jual.\n"; return false; }
    ...
    for (int i = 0; i < cartCount; ++i) {
        ...
        itemsDesc += cartCode[i] + ":" + to_string(cartQty[i]) + ":" + cartPriceStr[i];
    }
    ...
    double paid = 0.0;
    while (true) {
        string paidStr = inputLine("Masukkan jumlah tunai yang dibayar (angka): ");
        try { paid = stod(paidStr); } catch(...) { paid = 0.0; }
        if (paid < finalTotal) { cout << "Jumlah tidak cukup. Masukkan lagi.\n"; continue; }
        break;
    }
    double change = paid - finalTotal;

    // persist transaction with details
    saveTransactionToFile(curDate, total, itemsDesc, member, paid, change, totalDiscount, appliedPromoCode);
...
void viewTransactionDetails(int id) {
    ...
    while (getline(fin,line)) {
        ...
        if (pr=="y"||pr=="Y") printReceiptFile(id);
        break;
    }
    fin.close();
...
void cartMenu() {
    ...
    while (true) {
        ...
        if (ch == "1") {
            ...
            if (qty <= 0) { cout << "Jumlah tidak valid.\n"; continue; }
            if (addToCart(code, qty)) cout << "Barang ditambahkan ke keranjang.\n";
            ...
            if (removeCartItem(idx)) cout << "Item dihapus dari keranjang.\n";
            ...
            int qty = 0; for (size_t i=0;i<q.size();++i) if (q[i]>='0'&&q[i]<='9') qty = qty*10 + (q[i]-'0');
            if (updateCartQty(idx, qty)) cout << "Jumlah diperbarui.\n";
...
void bestSellingAnalysis() {
    ...
    // Sort by quantity (simple bubble sort)
    vector<pair<string,int>> sorted;
    for (auto& p : productSales) sorted.push_back(p);
    ...
    for (auto& p : sorted) {
        if (rank>10) break;
        cout<<rank<<" | "<<p.first<<" | "<<p.second<<" unit | "<<productCount[p.first]<<" transaksi\n";
        rank++;
    }
    cout<<"-------------------------------------------\n";
    cout<<"======================================\n";
...
void dailySalesReport() {
    ...
    double totalAll=0;
    for (auto& p : dailyRevenue) {
        cout<<p.first<<" | "<<dailyCount[p.first]<<" transaksi | Rp"<<(int)p.second<<"\n";
        totalAll+=p.second;
    }
    cout<<"-----------------------------------------------\n";
...
void categorySalesAnalysis() {
    ...
    while (getline(fprod,line)) {
        ...
        productCategory[pid]=cat;
    }
    fprod.close();
    
    // Calculate sales by category
    map<string,double> catRevenue;
    map<string,int> catQuantity;
    ...
    while (getline(fin,line)) {
        ...
        while (pos<items.length()) {
            ...
            if (productCategory.find(productId)!=productCategory.end()) {
                string cat=productCategory[productId];
                catQuantity[cat]+=qty;
                catRevenue[cat]+=(final*qty/100);
            }
            pos=pclose+1;
            if (pos<items.length() && items[pos]==',') pos++;
        }
    ...
    cout<<"----------------------------------------------------\n";
    double totalCat=0;
    for (auto& p : catRevenue) {
        cout<<p.first<<" | "<<catQuantity[p.first]<<" | Rp"<<(int)p.second<<"\n";
        totalCat+=p.second;
    }
    cout<<"----------------------------------------------------\n";
...
void memberSalesAnalysis() {
    ...
    string line;
    map<string,double> memberRevenue;
    map<string,int> memberCount;
    ...
    for (auto& p : memberRevenue) {
        cout<<p.first<<" | "<<memberCount[p.first]<<" | Rp"<<(int)p.second<<"\n";
    }
    cout<<"\nPenjualan Non-Member: "<<nonMemberCount<<" transaksi, Rp"<<(int)nonMemberRevenue<<"\n";
    double totalAllMember=0;
    for (auto& p : memberRevenue) totalAllMember+=p.second;
